ATTENTION: This file is a backup of README.rtf in case your computer cannot open rtf files. The images are missing as this is a .txt, but they are included in the zip.
...
Notes: There are quite a few files I decided to submit so I zipped them into a folder. If the images don’t appear in this file, they are also attached along with a text file version of this file (in case you cannot open .rtf files).
...
These results are clearly logarithmic, backing up the fact that insertion and removal are far better than O(n).

Proof Of Accuracy & Speed Through Test Cases
	The Main.java file is home to several test cases that will compare all three priority queue implementations mentioned before (java.util, both of mine). To test accuracy, I created the three types of priority queues and added 10 random strings to them (created with UUIDgen). They are then polled out and printed. The result is three lines containing the exact same series of UUID’s, proving the accuracy of my priority queues.
